...
Independent of other open tasks. Prerequisite for task-076 (auto-tuner).

## Goal

Stop discarding `BibCandidate` data after bib detection in the benchmark runner. Store a serialisable summary of all candidates (passed and rejected) in `PhotoResult` so downstream consumers (task-076 auto-tuner) can diagnose failures without re-running detection.
...
- **Out of scope**: consuming the data (that's task-076), modifying `detection/types.py`, web UI changes
