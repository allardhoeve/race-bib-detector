...
Reorganise `benchmarking/tuner.py` into a `benchmarking/tuners/` package with a `Tuner` Protocol so that the grid sweep and future auto-tuner (task-076) share a common interface.
...
| Modified | `todo/tasks/task-076-auto-tuner.md` | `auto_tuner.py` → `tuners/auto.py`, Tuner protocol conformance |
