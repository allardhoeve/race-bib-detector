...
- TODO: Replace ad-hoc `print()` calls across `scan/`, `sources/google_photos.py`, and `benchmarking/runner.py` with a shared logging setup so verbosity can be configured and tested.
...
- TODO: Add tests for `scan.is_photo_identifier()` and `run_scan()` routing (URL vs path vs hash) using mocks for `scan.scan_album`, `scan.scan_local_directory`, and `scan.rescan_single_photo`.
