...
def test_roundtrip_with_links():
    gt = LinkGroundTruth()
    gt.set_links("abc123", [BibFaceLink(bib_index=0, face_index=1), BibFaceLink(bib_index=2, face_index=0)])
...
def test_set_and_get_links():
    gt = LinkGroundTruth()
    links = [BibFaceLink(bib_index=1, face_index=3), BibFaceLink(bib_index=0, face_index=0)]
...
def test_save_load_roundtrip(tmp_path):
    gt = LinkGroundTruth()
    gt.set_links("deadbeef", [BibFaceLink(bib_index=0, face_index=2)])
