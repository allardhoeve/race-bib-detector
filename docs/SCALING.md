...
Currently images are decoded up to 3 times per photo (bib detection, face detection, face embedding). After task-069, this reduces to 1 time. At 10,000 photos with 2MB average, the image cache holds ~20 GB. This exceeds RAM on most machines.
...
`PhotoResult` stores traces, boxes, scorecards, and (after task-069) embedding vectors per photo. At current scale this is ~10 MB. At 10,000 photos it could reach 500 MB-1 GB.
